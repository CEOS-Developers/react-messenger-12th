...
const DMLIST = [
  {
    ...
    name: '가유빈',
    id:"gayubin",
    content: messageData.gayubin.chat[messageData.gayubin.chat.length-1].content,
  },
  {
    img:
    ...
    name: "나유빈",
    id:"nayubin",
    content: messageData.nayubin.chat[messageData.nayubin.chat.length-1].content,
  },
  {
    img:
    ...
    name: "다유빈",
    id:"dayubin",
    content: messageData.dayubin.chat[messageData.dayubin.chat.length-1].content,
  },
];
...
const DMListScreen = () => {
  ...
  const [filteredDMFriends, setFilteredDMFriends] = useState([]);
  const [selectedIndex, setSelectedIndex] = React.useState();

  useEffect(() => {
    setFilteredDMFriends(
      DMList.filter((followers) => {
        console.log('followers', DMList);
        return followers.name.toLowerCase().includes(search.toLowerCase());
      })
    );
  }, [search, DMList]);
  ...
  return (
    <Wrapper>
      ...
      <SearchBox type='text' placeholder='search' onChange={handleChange} size="50"></SearchBox>
      <Profile>
        
        {filteredDMFriends.map((info, index) => {
          return (
          
            <div key={index}>
             <StyledLink to= {`/chat/:${info.id}`}>
              <ProfileCard
              ...
              >
                ...
                <TouchableOpacity activeOpacity={0.5}>
                  <Image
                    source={{
                      uri:
                        'https://cdn.iconscout.com/icon/premium/png-256-thumb/trash-can-1778449-1515973.png',
                    }}
                  />
                </TouchableOpacity>
              </ProfileCard>
